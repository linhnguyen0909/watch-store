...
<body>
									...
									<ul class="wsmenu-list">
										<li class="cart-item has-icon">
											<div class="shopping-cart" id="cart_button">
												...
												</a> <span id="length">${sessionScope.countTotalQuantity}</span>
											</div>
				...
				<div class="container">
					...
					<div class="shopping-table">
							<table class="table">
								...
								<tbody id="wrap-Cart">
									<c:forEach items="${sessionScope.cart.listDetail}"
												var="product">
										<tr>
											<td>${product.value.name}</td>
											<td>${product.value.itemPrice}</td>
											<td>
												<input
													class="quantityProd"
													id="quantity${product.key}" 
													type="number" min="0" value="${product.value.quantity}"
													onchange="updateQuantity(${product.key})"
													/>
											</td>
											<td>${product.value.totalPrice}</td>
											<td>
												<a href="/gio-hang/updateProductInCartPage?id=${product.key}&quantity=0">
	...
	<script>
		function updateQuantity(productId) {
			...
			var url = "/gio-hang/updateProductInCartPage?id=" + productId + "&quantity=" + newQuantity;
